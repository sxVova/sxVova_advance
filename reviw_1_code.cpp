...
{
    std::vector<int>::iterator middle_separator;
    ...
    while (cut_length > 0) {
        middle_separator = place_to_insert;
...
std::vector<int> find_optimal_subsequence(const std::vector<int>& employee_demand_ratios)
{
    ...
    for (int current = 0; current < vector_size; ++current) 
    {
        ...
        pointer_to_the_place_to_insert = find_lower_bound(
            ...
            reversed_employee_ratios[current]
        );

        insertion_index += pointer_to_the_place_to_insert - largest_number_sequence_ends.begin();
        
        if (
            largest_number_sequence_ends[insertion_index - 1] <= 
            reversed_employee_ratios[current] &&
            reversed_employee_ratios[current] <= 
            largest_number_sequence_ends[insertion_index]
            ) // bid validation
        {
            largest_number_sequence_ends[insertion_index] = reversed_employee_ratios[current];
